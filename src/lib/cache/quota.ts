...
export const userRecencyKey = (userId: string) =>
	`cache:v1:user:${userId}:recency`; // ZSET: member=chatId, score=lastAccessMillis
export const userSizesKey = (userId: string) => `cache:v1:user:${userId}:sizes`; // HASH: field=chatId, value=bytes (stringified int)
export const userUsedBytesKey = (userId: string) =>
	`cache:v1:user:${userId}:used_bytes`; // STRING: int bytes
// SET of pinned chatIds for a user; membership used to prefer keeping pinned in cache
export const userPinnedKey = (userId: string) =>
	`cache:v1:user:${userId}:pinned`;
...
export async function heartbeat(
	userId: string,
): Promise<{ activeUsers: number; perUserBudgetBytes: number }> {
	const now = Date.now();
...
export async function recordChatSize(
	userId: string,
	chatId: string,
	newSizeBytes: number,
...
export async function ensureUnderQuota(
	userId: string,
	budgetBytes: number,
): Promise<{ evicted: Array<{ chatId: string; bytes: number }> }> {
	...
	while (used > budgetBytes && safety-- > 0) {
		...
		for (const candidate of candidates) {
			if (!pinnedSet.has(candidate)) {
				victimChatId = candidate;
				break;
			}
		}
		if (!victimChatId) {
