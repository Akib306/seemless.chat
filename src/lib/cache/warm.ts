...
export async function warmUserCacheIfNeeded(
	...
	try {
		...
		const lockAcquired = await redis.set(warmLockKey, "1", {
			nx: true,
			ex: 30,
		});
		...
		try {
			...
			const WARM_CHATS_LIMIT = Math.max(
				1,
				Number(process.env.CACHE_WARM_CHATS_LIMIT || 8),
			);
			...
			if (targets.length > 0) {
				...
				const existence = await Promise.all(
					toFetch.map(({ key }) => redis.exists(key)),
				);
					...
					await Promise.all(
						missing.map(async ({ chatId }) => {
							messagesByChat[chatId] =
								await db.messages.getMessagesByChatId(chatId);
						}),
					);
			...
			await redis.set(warmMarkerKey, String(Date.now()), {
				ex: ONE_DAY_SECONDS,
			});
		} finally {
			// Let the lock expire
