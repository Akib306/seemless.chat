-- Update search functions to support partial matching
-- This migration improves the search functionality to find partial matches like "leb" in "LeBron"

-- Function to search messages and return results with chat context (updated for partial matching)
...
BEGIN
  ...
  WHERE 
    m.user_id = user_uuid
    AND (
      m.content_search @@ plainto_tsquery('english', search_query)
...
-- Function to search messages with pagination (updated for partial matching)
...
-- Function to get search result counts (updated for partial matching)
